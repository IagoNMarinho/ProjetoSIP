...
class StorageService {
  ...
  updateAnalysis(updated: Analysis): void {
    ...
    const index = history.findIndex((item) => item.id === updated.id);
  ...
  getStatistics() {
    //função para captar a quantidade de analises
    const history = this.getHistory();
    return {
      total: history.length,
      potavel: history.filter((item) => item.status === "Potável").length,
      atencao: history.filter((item) => item.status === "Atenção").length,
      critica: history.filter((item) => item.status === "Crítica").length,
    };
  }
